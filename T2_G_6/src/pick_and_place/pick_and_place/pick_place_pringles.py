...
from control_msgs.action import FollowJointTrajectory # <-- L'azione standard per le traiettorie
...
class TiagoPickAndPlace(Node):
    def __init__(self):
        ...
        self.create_subscription(PoseStamped, '/aruco_base_1', self.pick_marker_callback, 10)
        self.create_subscription(PoseStamped, '/aruco_base_4', self.place_marker_callback, 10)
    ...
    def on_retreat_reached(self, future):
        """Chiamata quando RETREAT1 (sollevamento del torso) è completato"""
        self.get_logger().info("RETREAT1 completato. Avvio RETREAT2 cartesiano...")
        self.state = "RETREAT2"
        
    def on_retreat2_reached(self, future):
        """Chiamata quando RETREAT2 (sollevamento cartesiano del braccio) è completato"""
        self.get_logger().info("RETREAT2 completato. Allontanamento riuscito con successo!")
        self.state = "RETREAT3"

    def on_retreat2_step_reached(self, future):
        # Callback intermedia per ogni passo da 2 cm
        self.get_logger().info("Step di 2 cm raggiunto. Calcolo il prossimo...")
        
        if self.current_retreat_dz < 0.15:
            self.state = "RETREAT2"
        else:
            self.state = "WAITING"
            self.on_retreat2_reached(future)
            
            
    def on_retreat3_reached(self, future):
        """Chiamata quando RETREAT3 (sollevamento cartesiano del braccio) è completato"""
        self.get_logger().info("RETREAT3 completato. Allontanamento riuscito con successo!")
        self.state = "DONE" 
    ...
    def loop(self):
        ...
        if self.state == "INIT":
            ...
            pos_above = self._get_offset_position(self.pick_pose, dx=-0.0035, dy=-0.045, dz= -0.09) 
            ...
            pos_lower = self._get_offset_position(self.pick_pose, dx=-0.0035, dy=-0.045, dz=-0.19) #CAMBIO
            ...
            # Calcolo la cinematica inversa
            q_lift = self.robot.ik_NR(TF_lift, q0=self.q_sent, pinv=True)[0]
            self.q_sent = q_lift
            
            # Invio il comando al braccio e aggancio la nuova callback
        ...
        ############################PLACE
            ...
            if self.place_pose is None:
                # Usa throttle_duration_sec per stampare l'avviso solo 1 volta ogni 2 secondi
            ...
            pos_place_above = self._get_offset_position(self.place_pose, dx=0, dy=0, dz=0.07)
            ...
            self.state = "WAITING"
            self.get_logger().info("RETREAT1: Alzo il torso...")

            q_retreat = self.q_sent.copy()
            q_retreat[0] = 0.35 
            self.q_sent = q_retreat
            
            self.send_arm_torso_goal(q_retreat, callback=self.on_retreat_reached)
            
        elif self.state == "RETREAT2":
            STEP_SIZE_J3 = 0.02  
            STEP_SIZE_J6 = -0.04 
            STEP_SIZE_J7 = 0.02   
            
            TARGET_STEPS = 15 
            PASSO_OGNI_TOT_SECONDI = 5.0  
            
            if not hasattr(self, 'retreat_step_count'):
                self.retreat_step_count = 0

            tempo_attuale = self.get_clock().now()
            if not hasattr(self, 'last_step_time') or self.last_step_time is None:
                self.last_step_time = tempo_attuale

            tempo_trascorso = (tempo_attuale - self.last_step_time).nanoseconds / 1e9 # In secondi
            
            if tempo_trascorso < PASSO_OGNI_TOT_SECONDI:
                return

            self.last_step_time = tempo_attuale

            if self.retreat_step_count >= TARGET_STEPS:
                self.retreat_step_count = 0
                
                if hasattr(self, 'current_joint_positions') and self.current_joint_positions is not None:
                    self.q_sent = list(self.current_joint_positions)
                # --------------------------------
                
                self.state = "RETREAT3" 
                self.get_logger().info("RETREAT2 Completato! Riallineamento giunti eseguito. Passo a RETREAT3.")
                return

            self.retreat_step_count += 1
            self.get_logger().info(f"RETREAT2: Passo giunti {self.retreat_step_count}/{TARGET_STEPS}...")

            if hasattr(self, 'current_joint_positions') and self.current_joint_positions is not None:
                q_next = list(self.current_joint_positions)
            else:
                q_next = list(self.q_sent)


            q_next[2] += STEP_SIZE_J3
            q_next[5] += STEP_SIZE_J6
            q_next[6] += STEP_SIZE_J7 
            
            self.q_sent = q_next
            
            self.send_arm_torso_goal(q_next, callback=lambda future: None)
            
            
            
        elif self.state == "RETREAT3":
            self.get_logger().info("RETREAT3: Preparazione movimento diretto su Giunto 1 e Giunto 3...")

            q_target = list(self.q_sent)
            ...
            self.get_logger().info("RETREAT3: Invio comando per Giunto 1 -> 0.8 e Giunto 3 -> -1.5")
            self.send_arm_torso_goal(q_target, callback=self.on_retreat3_reached)
            

            #self.state = "MOVING_TO_RETREAT3"
    ...
    def attach_object(self):
        ...
        req.model_name_2, req.link_name_2 = 'pringles', 'link'
